...
class LanguageModel(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, n_layers=1, dropout_p=0.5,SEQ_LENGTH = 30):
        super(LanguageModel, self).__init__()
        self.n_layers = n_layers
        self.hidden_size = hidden_size
        self.embed = nn.Embedding(vocab_size, embedding_size)
        # self.rnn = nn.LSTM(embedding_size, hidden_size, n_layers, batch_first=True)
        self.rnn = simpleLSTM(embedding_size, hidden_size, n_layers, batch_first=True,SEQ_LENGTH=SEQ_LENGTH)
...
model = LanguageModel(len(word2index), EMBED_SIZE, HIDDEN_SIZE, NUM_LAYER, 0.5,SEQ_LENGTH)
model.init_weight()
if USE_CUDA:
    model = model.cuda()
